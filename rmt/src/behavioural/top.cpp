...
top::top(sc_module_name nm,
  ...
  : topSIM(nm, parent, configfile), DebuggerUtilities() {
  /*sc_spawn threads*/
...
void top::init() {
    ...
    if (PFP_DEBUGGER_ENABLED) {
      std::cerr << "\n" << On_IRed
                << "ERROR: Please compile with PFPSIM_DEBUGGER flag for "
                << "access to the debugger. Use: cmake -DPFPSIMDEBUGGER=ON "
                << "../src/ in the build directory." << txtrst << std::endl;
      return 1;
    }
  #endif
