...
PacketDescriptor::PacketDescriptor(std::size_t id, std::size_t packet_context,
      ...
      std::size_t packet_rcos, std::size_t priority)
      : pfp::core::PacketBase(id),
        ...
        packet_header_(P4::get("npu")-> new_packet_ptr(
         0/*ingress_port*/, id,
         raw_header.size(),
         bm::PacketBuffer(
            raw_header.size(),  // capacity (TODO(gb) be less arbitrary)
            raw_header.data(),
            raw_header.size()))) {
}
