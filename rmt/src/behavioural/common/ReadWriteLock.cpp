...
ReadWriteLock::ReadWriteLock() : writer(nullptr) {
}
...
void ReadWriteLock::read_lock() {
  ...
  npulog(profile, cout << this_proc->name() << " attempting to get read lock@"
        << this << endl;)
...
void ReadWriteLock::read_unlock() {
  ...
  npulog(profile, cout << this_proc->name() << " released read lock@" << this
        << endl;)
...
void ReadWriteLock::write_lock() {
  ...
  npulog(profile, cout << this_proc->name() << " attempting to get write lock@"
        << this << endl;)
  ...
  npulog(profile, cout << this_proc->name() << " got write lock@" << this
        << endl;)
}
...
void ReadWriteLock::write_unlock() {
  ...
  npulog(profile, cout << this_proc->name() << " released write lock@" << this
        << endl;)
