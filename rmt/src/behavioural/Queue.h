...
public QueueSIM {
 public:
  /* CE Constructor */
  explicit Queue(sc_module_name nm, pfp::core::PFPObject* parent = 0,
  ...
  // Peek interface
  virtual T peek( tlm_tag<T> *t = 0 ) const { return fifo_.peek(t);}
  ...
  virtual void write(const T& t) {fifo_.put(t);}
 ...
 */
template<typename T>
Queue<T>::Queue(sc_module_name nm, pfp::core::PFPObject* parent,
      std::string configfile): QueueSIM(nm, parent, configfile),
